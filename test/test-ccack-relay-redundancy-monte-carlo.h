...
#ifndef CCACK_TEST_CCACK_RELAY_REDUNDANCY_MONTE_CARLO_H_
#define CCACK_TEST_CCACK_RELAY_REDUNDANCY_MONTE_CARLO_H_
...
void TestCcackRelayRedundancyMonteCarlo() {
	...
	while (numit != ++counter) {
		...
		do {
			...
			//
			// add redundancy equal to the expectation value of the losses
			//
			uint16_t toSend = symbols * 2;
			while (toSend-- != 0) {
			...
			//
			// send additional redundancy
			//
			toSend = (encoder->rank() - heardS) * 2;
...
#endif /* CCACK_TEST_CCACK_RELAY_REDUNDANCY_MONTE_CARLO_H_ */
